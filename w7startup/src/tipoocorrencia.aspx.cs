...
namespace global
{
    public partial class tipoocorrencia : System.Web.UI.Page
    {
        ...
        protected void btnSalvar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(hdfId.Value))
                {
                    ...
                    string updateQuery = "UPDATE OniPres_tipoOcorrencia SET nome = @nome, [status] = @status WHERE id = @id";
                ...
                else
                {
                    ...
                    string insertQuery = "INSERT INTO OniPres_tipoOcorrencia (nome, [status]) VALUES (@nome, @status)";
        ...
        {
            sdsDados.SelectCommand = "select id, nome, status from OniPres_tipoOcorrencia where [status] = 'Ativo' and nome like '%" + txtBuscar.Text + "%'";
            BindData();
        }
        ...
        private void ExcluirRegistro(int id)
        {
            ...
            Database db = DatabaseFactory.CreateDatabase("ConnectionString");
            string query = "DELETE FROM OniPres_tipoOcorrencia WHERE id = @id";
        ...
        {
            Database db = DatabaseFactory.CreateDatabase("ConnectionString");
            string query = "SELECT id, nome FROM OniPres_tipoOcorrencia WHERE id = @id";
            DbCommand cmd = db.GetSqlStringCommand(query);
            db.AddInParameter(cmd, "@id", DbType.Int32, id);
